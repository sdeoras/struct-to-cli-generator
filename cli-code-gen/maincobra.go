...
func main() {
	...
	printFields(reflect.Indirect(reflect.ValueOf(config)), true, "config", "Configure cluster",
...
func getFunc(cw, fw *bufio.Writer, v reflect.Value, i int, tab, prefix string) {
	...
	fmt.Fprintln(fw, "\t", "set, err = provider.IsSet(\""+getTag(v.Type(), i)+"\")")
	fmt.Fprintln(fw, "\t", "if err != nil { return err }")
	fmt.Fprintln(fw, "\t", "if set || setAll {")
	fmt.Fprintln(fw, "\t\t",
...
func setFunc(cw, fw *bufio.Writer, v reflect.Value, i int, tab, prefix, castType, flagType, flagTag string) {
	...
	fmt.Fprintln(fw, "\t", "if set, err := provider.IsSet(\""+getTag(v.Type(), i)+"\"); err != nil {")
	fmt.Fprintln(fw, "\t\t", "return err")
	fmt.Fprintln(fw, "\t", "} else {")
	fmt.Fprintln(fw, "\t\t", "if set {")
	fmt.Fprintln(fw, "\t\t\t",
		"if value, err :=", "provider.Get"+flagType+"(\""+getTag(v.Type(), i)+"\"); err != nil {")
	fmt.Fprintln(fw, "\t\t\t\t", "return err")
	fmt.Fprintln(fw, "\t\t\t",
		"} else {")
	if castType == "" {
		fmt.Fprintln(fw, "\t\t\t\t",
			prefix2VarName[prefix]+"."+v.Type().Field(i).Name, "=", "value")
	} else {
		fmt.Fprintln(fw, "\t\t\t\t",
			prefix2VarName[prefix]+"."+v.Type().Field(i).Name, "=", castType+"(value)")
	}
	fmt.Fprintln(fw, "\t\t\t", "}")
	fmt.Fprintln(fw, "\t\t", "}")
	fmt.Fprintln(fw, "\t", "}")
...
func printFields(v reflect.Value, hidden bool, prefix, usage, description, tab string) {
	...
	fmt.Fprintln(fw, "//", getCamelCase("set_"+prefix+"_values"), "sets config values using codegangsta cli context.")
	fmt.Fprintln(fw, "// This func is autogenerated. Please DO NOT EDIT.")
	fmt.Fprintln(fw, "func", getCamelCase("set_"+prefix+"_values(c *cli.Context) error {"))
	fmt.Fprintln(fw, "\t provider, err := cflags.NewCodeGangstaProvider(c)")
	fmt.Fprintln(fw, "\t if err != nil {")
	fmt.Fprintln(fw, "\t\t return err")
	fmt.Fprintln(fw, "\t }")
	fmt.Fprintln(fw, "\t return", getCamelCase("Set_"+prefix+"_values(provider)"))
	fmt.Fprintln(fw, "}")
	fmt.Fprintln(fw)
	fmt.Fprintln(fw, "//", getCamelCase("Set_"+prefix+"_values"), "sets config values using cflags provider.")
	fmt.Fprintln(fw, "// This func is autogenerated. Please DO NOT EDIT.")
	fmt.Fprintln(fw, "func", getCamelCase("Set_"+prefix+"_values(provider cflags.Provider) error {"))
	if prefixOrigin[prefix] == "node" {
		fmt.Fprintln(fw, "\t", "var setNodeID bool")
		fmt.Fprintln(fw, "\t", "var setAll bool")
		fmt.Fprintln(fw, "\t", "var err error")
		fmt.Fprintln(fw, "\t", "setNodeID, err = provider.IsSet(\"node_id\")")
		fmt.Fprintln(fw, "\t", "if err != nil {")
		fmt.Fprintln(fw, "\t\t", "return err")
		fmt.Fprintln(fw, "\t", "}")
		fmt.Fprintln(fw, "\t", "setAll, err = provider.IsSet(\"all\")")
		fmt.Fprintln(fw, "\t", "if err != nil {")
		fmt.Fprintln(fw, "\t\t", "return err")
		fmt.Fprintln(fw, "\t", "}")
		fmt.Fprintln(fw, "\t", "if !setNodeID && !setAll {")
		...
		fmt.Fprintln(fw, "\t\t", "}")
		fmt.Fprintln(fw, "\t", "} else {")
		fmt.Fprintln(fw, "\t\t", "if nodeID, err := provider.GetGlobalString(\"node_id\"); err != nil {")
		fmt.Fprintln(fw, "\t\t\t", "return err")
		fmt.Fprintln(fw, "\t\t", "} else {")
		fmt.Fprintln(fw, "\t\t\t", "if config, err := clusterManager.GetNodeConf(nodeID); err != nil {")
		fmt.Fprintln(fw, "\t\t\t\t", "logrus.Error(err)")
		fmt.Fprintln(fw, "\t\t\t\t", "return err")
		fmt.Fprintln(fw, "\t\t\t", "} else {")
	...
	fmt.Fprintln(fw, "//", getCamelCase("show_"+prefix+"_values"), "retrieves config values using codegangsta cli context.")
	fmt.Fprintln(fw, "// This func is autogenerated. Please DO NOT EDIT.")
	...
	fmt.Fprintln(fw, "\t\t return err")
	fmt.Fprintln(fw, "\t }")
	fmt.Fprintln(fw, "\t return", getCamelCase("Show_"+prefix+"_values(provider)"))
	fmt.Fprintln(fw, "}")
	fmt.Fprintln(fw)
	fmt.Fprintln(fw, "//", getCamelCase("Show_"+prefix+"_values"), "retrieves config values using codegangsta cli context.")
	fmt.Fprintln(fw, "// This func is autogenerated. Please DO NOT EDIT.")
	fmt.Fprintln(fw, "func", getCamelCase("Show_"+prefix+"_values(provider cflags.Provider) error {"))
	if prefixOrigin[prefix] == "node" {
		fmt.Fprintln(fw, "\t", "var setNodeID bool")
		fmt.Fprintln(fw, "\t", "var setAll bool")
		fmt.Fprintln(fw, "\t", "var err error")
	...
	fmt.Fprintln(fw, nullChecker(prefix2VarName[prefix]+".field"))
	fmt.Fprintln(fw, "\t", "if jsonOut, err := provider.GetGlobalBool(\"json\"); err != nil {")
	fmt.Fprintln(fw, "\t\t", "return err")
	fmt.Fprintln(fw, "\t", "} else {")
	fmt.Fprintln(fw, "\t\t", "if jsonOut {")
	if prefixOrigin[prefix] == "node" {
	...
	fmt.Fprintln(cw, tabs(tab, 4), "},")
	if v.NumField() > 0 {
		fmt.Fprintln(fw, "\t", "var set bool; var setAll bool")
		fmt.Fprintln(fw, "\t", "setAll, err = provider.IsSet(\"all\")")
		fmt.Fprintln(fw, "\t", "if err != nil { return err }")
	}
	for i := 0; i < v.NumField(); i++ {
